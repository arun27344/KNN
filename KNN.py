...
def knn(k_value,temperature,moisture, wind_speed,air_quality):
    train = pd.read_csv('weather.csv')
    # read data from file
    cols = ['Temperature', 'Moisture', 'Wind_speed', 'Air_quality']
    train.rename(columns = {cols[0]:0, cols[1]:1, cols[2]:2, cols[3]:3}, inplace=True)
    train['distance'] = 9999
    # # rename columns header and set new column distances
    target = [temperature, moisture, wind_speed,air_quality] #pd.Series([30, 200, 180])
